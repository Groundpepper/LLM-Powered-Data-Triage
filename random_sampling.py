...
class RandomSampler:
    ...
    def get_sample(self, data, size):
        if data.empty: return pd.DataFrame()
        else: return data.sample(min(size, len(data)), random_state=42)
        ...
        for cluster in unique_clusters:
            ...
            if filter_label:
                if "predicted_label" in cluster_data.columns:
                    ...
                    pos_cluster_data = self.get_sample(pos, n_sample)
                    neg_cluster_data = self.get_sample(neg, samples_per_cluster - len(pos_cluster_data))
                    sampled_data.append(pd.concat([pos_cluster_data, neg_cluster_data]).sample(frac=1))

                else:
                    sampled_data.append(self.get_sample(cluster_data, size=samples_per_cluster))
